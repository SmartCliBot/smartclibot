    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "<i>اخطار!!!\n  اسپم مجاز نیست❌\n کاربر " ..string.gsub(msg.from.print_name, '_', ' ').."\n به دلیل اقدام به خرابکاری اخراج شد.\n</i>--| @Radicall_Team |--")
		else
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "کاربر ["..string.gsub(msg.from.print_name, '_', ' ').."]\nبه دلیل اقدام های مکرر برای خرابکاری بصورت اتوماتیک از تمامی گروه ها مسدود شد (spamming)")
		  send_large_msg("channel#id"..msg.to.id, "کاربر ["..string.gsub(msg.from.print_name, '_', ' ').."]\nبه دلیل اقدام های مکرر برای خرابکاری بصورت اتوماتیک از تمامی گروه ها مسدود شد (spamming)")
          local GBan_log = 'GBan_log'
		  local GBan_log =  data[tostring(GBan_log)]
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "کاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.."به دلیل اقدام به خراب کاری در گروه "..msg.to.print_name.." ) [ "..msg.to.id.." ]بصورت اتوماتیک از تمامی گروه ها مسدود شد"
