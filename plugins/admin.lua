...
local function run(msg,matches)
    ...
    if matches[1] == "تیک دوم" then
    	if matches[2] == "روشن" then
    		...
    		return "<i>تیک دوم > روشن</i>"
    	end
    	if matches[2] == "خاموش" then
    		redis:del("bot:markread")
    		return "<i>تیک دوم > خاموش</i>"
    	end
    	return
    end
    if matches[1] == "ارسال پیام" then
    	local text = "سازنده ربات برای شما پیامی ارسال کرده است\n\nمتن پیام:\n\n"..matches[3].."\n\n----------------------------------\n--| @Radicall_team |--"
    	send_large_msg("user#id"..matches[2],text)
    	return reply_msg(msg.id, '<i>با موفقیت ارسال شد</i>', ok_cb, false)
    end
    
    if matches[1] == "بلاک پی وی" then
    	if is_admin2(matches[2]) then
    		return "<i>من نمیتوانم ادمین ربات را بلاک کنم!</i>"
    	end
    	block_user("user#id"..matches[2],ok_cb,false)
    	return "<i>کاربر مورد نظر بلاک شد</i>"
    end
    if matches[1] == "انبلاک پی وی" then
    	unblock_user("user#id"..matches[2],ok_cb,false)
    	return "<i>کاربر مورد نظر انبلاک شد!</i>"
    end
    if matches[1] == "برو به" then--join by group link
    	local hash = parsed_url(matches[2])
    	import_chat_link(hash,ok_cb,false)
    end
    if matches[1] == "اطلاعات کاربران" then
      if not is_sudo(msg) then-- Sudo only
    		return 
  		end
      get_contact_list(get_contact_list_callback, {target = msg.from.id})
      return reply_msg(msg.id, '<i>سازنده ربات شناسایی شد👤\nاطلاعات تمامی کاربران ربات به خصوصی شما ارسال شد</i>', ok_cb, false)
    end
 ...
 if matches[1] == "mycontact" and is_sudo(msg) then
	...
	if matches[1] == 'reload' then
		...
		reload_plugins(true)
		post_msg(receiver, "ربات با موفقیت بارگذاری شد", ok_cb, false)
		return "--| @Radicall_team |--"
	end
	--[[*For Debug*
