...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "<i>اطلاعاتی درباره این کاربر یافت نشد</i>")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         if member_id == from_id then
            send_large_msg(receiver, "<i>شما نمیتوانید خود را اخراج کنید</i>")
			return
         end
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "<i>شما نمیتوانید مدیران مسدود کنید</i>")
			return
         end
		 kick_user(member_id, chat_id)
      elseif get_cmd == 'ban' then
        if is_momod2(member_id, chat_id) and not is_admin2(sender) then
			send_large_msg(receiver, "<i>شما نمیتوانید مدیران را مسدود کنید</i>")
			return
        end
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از گروه مسدود شد🚫')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از مسدود های گروه خارج شد')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از همه گروه های ربات مسدود شد🚫')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از همه گروه های مسدود شده آزاد شد')
  ...
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "<i>شما نمیتوانید مدیران را مسدود کنید</i>"
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "<i>شما نمیتوانید خود را مسدود کنید</i>"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
		...
		send_large_msg(receiver, 'کاربر ['..matches[2]..'] از گروه مسدود شد🚫')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'ban',
  ...
  if matches[1]:lower() == 'unban' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return 'کاربر '..user_id..' از مسدود های گروه خارج شد'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unban',
...
if matches[1]:lower() == 'kick' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "<i>شما نمیتوانید مدیران را اخراج کنید</i>"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "<i>شما نمیتوانید خود را اخراج کنید</i>"
		end
    local user_id = matches[2]
    local chat_id = msg.to.id
		local print_name = user_print_name(msg.from):gsub("‮", "")
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'کاربر ['..user_id..' ] از همه گروه های ربات مسدود شد🚫'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] از گروه های مسدود شده آزاد شد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
